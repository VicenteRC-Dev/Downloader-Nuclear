...
// Función automática para obtener la IP IPv4 local de la máquina
function obtenerIPLocal() {
  const interfaces = os.networkInterfaces();
  for (const nombreInterface of Object.keys(interfaces)) {
    for (const detalles of interfaces[nombreInterface]) {
      if (detalles.family === 'IPv4' && !detalles.internal) {
        return detalles.address;
      }
    }
  }
  return '127.0.0.1';
...
const TARGET_HOST = process.env.TARGET_HOST || obtenerIPLocal();
